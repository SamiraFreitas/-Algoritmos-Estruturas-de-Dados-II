...
void intercalacao_arvore_de_vencedores(TPilha **pilha, int *vetTop, char *nome_arquivo_saida, int num_p){
    ...
    TNo_vencedor **backup_endereco_p = (TNo_vencedor**)malloc(sizeof(TNo_vencedor*)*num_p);
    ...
    if ((out = fopen(nome_arquivo_saida, "wb")) == NULL) {
        ...
        raiz = cria_arvore_vencedores_inicial(pilha, vetTop, num_p, backup_endereco_p);

        for(i=0; i<20; i++){
            fseek(out, i* tamanho_registro(), SEEK_SET);
            salva_funcionario(raiz->vencedor, out);
            reorganiza_arvore(raiz, pilha, vetTop, num_p, backup_endereco_p);
        }
